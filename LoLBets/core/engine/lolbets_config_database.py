"""LoLBets configuration database for per-guild setup and integrations."""
...
class HexbetConfigDB:
    """Database manager for LoLBets guild configurations."""
    ...
    def _ensure_tables(self):
        ...
        with self.get_connection() as conn:
            with conn.cursor() as cur:
                ...
                conn.commit()
                logger.info("✅ LoLBets config tables ensured")
    ...
    def get_guild_config(self, guild_id: int) -> Optional[Dict]:
        ...
        with self.get_connection() as conn:
            with conn.cursor(cursor_factory=RealDictCursor) as cur:
                cur.execute("""
                    SELECT * FROM lolbets_guild_config WHERE guild_id = %s
                """, (guild_id,))
                return cur.fetchone()
        ...
        with self.get_connection() as conn:
            with conn.cursor() as cur:
                ...
                if exists:
                    ...
                    if updates:
                        query = f"UPDATE lolbets_guild_config SET {', '.join(updates)} WHERE guild_id = %s"
...
def get_hexbet_config_db() -> HexbetConfigDB:
    """Compatibility helper used by the forked engine."""
    return HexbetConfigDB()


def get_lolbets_config_db() -> HexbetConfigDB:
    return HexbetConfigDB()
