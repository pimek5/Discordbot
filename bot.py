...
async def on_ready():
    ...
    # Send profile announcement embed (one-time)
    try:
        announcement_channel = bot.get_channel(1153035389196570674)
        if announcement_channel:
            embed = discord.Embed(
                title="📢 HEXRTBRXENBOT - Profile System & Application Usage",
                description="HEXRTBRXENBOT can now be added as a **User Install Application**! This means you can use many commands directly in DMs or on any server where the bot is present.",
                color=0x2B2D31
            )
            
            embed.add_field(
                name="👤 PROFILE SYSTEM - Getting Started",
                value=(
                    "**1. Link Your Riot Account**\n"
                    "`/link riot_id:YourName#TAG region:[Region]`\n\n"
                    "**2. Verify Your Account**\n"
                    "Change your **League of Legends profile icon** to the icon shown in the verification message, then use:\n"
                    "`/verifyacc`\n"
                    "• Bot will check if your icon matches the verification code\n"
                    "• Once verified, your account is permanently linked!\n\n"
                    "**3. View Your Profile**\n"
            ...
            embed.add_field(
                ...
                value=(
                    ...
                    "`/link riot_id:SecondAccount#EUW region:euw`\n\n"
                    "**Manage Your Accounts**\n"
                    "`/accounts` - View all linked accounts, hide/show accounts, delete unwanted accounts\n\n"
                    "**Set Primary Account**\n"
                    "`/setmain` - Choose which account displays first in your profile\n\n"
                    "**Unlink an Account**\n"
                    "`/unlink` - Select an account to remove from your profile"
                ),
                inline=False
            )
            ...
            embed.add_field(
                name="✨ AND MORE",
                value=(
                    "`/stats` - Recent match statistics\n"
                    "`/matches` - Match history from all accounts\n"
                    "`/points` - TOP 10 champion masteries\n"
                    "`/compare` - Compare mastery between players\n"
                    "`/top` - Champion leaderboards\n"
                    "`/ranktop` - Ranked leaderboard\n\n"
                    "Use `/help` or `/commands` for full command list!"
                ),
                inline=False
            )
            
            embed.set_footer(text="⚙️ Multi-account aggregation • Real-time Riot API data • Privacy controls")
            ...
            print(f"✅ Sent profile announcement to channel {announcement_channel.name}")
    except Exception as e:
        print(f"⚠️ Failed to send announcement: {e}")
