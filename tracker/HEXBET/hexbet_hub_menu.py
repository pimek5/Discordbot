"""
HEXBET Hub Menu System - Consolidates commands into embed-based navigation
Central dashboard for all betting, player, and account operations
"""
...
logger = logging.getLogger('hexbet_hub')
...
class HexbetMenuSelect(ui.Select):
    ...
    async def _show_play_menu(self, interaction: discord.Interaction):
        ...
        embed.add_field(
            name="🎯 Find & Post Game",
            value="`/hxfind` [platform] [nickname]\nFind and post a high-elo game",
            inline=False
        )
        
        embed.add_field(
            name="📋 View Open Bets",
            value="`/hxrefresh`\nRefresh all open bet embeds & recalculate odds",
            inline=False
        )
    ...
    async def _show_players_menu(self, interaction: discord.Interaction):
        ...
        embed.add_field(
            name="🔍 Search Player",
            value="`/hxplayer <name> [region]`\nCheck player profile & pro status",
            inline=False
        )
    ...
    async def _show_account_menu(self, interaction: discord.Interaction):
        ...
        embed.add_field(
            ...
            value="`/hxstats` [@user]\nView betting statistics & badges",
            inline=False
        )
...
class PlaySubMenu(ui.View):
    ...
    async def find_game(self, interaction: discord.Interaction, button: ui.Button):
        """Quick link to /hxfind"""
        await interaction.response.send_message(
            "📝 Use `/hxfind` command:\n```\n/hxfind [platform] [nickname]\n```\n"
    ...
    async def refresh_odds(self, interaction: discord.Interaction, button: ui.Button):
        await interaction.response.send_message(
            "Use `/hxrefresh` to update all open bets and recalculate odds",
            ephemeral=True
        )
...
class PlayersSubMenu(ui.View):
    ...
    async def search(self, interaction: discord.Interaction, button: ui.Button):
        await interaction.response.send_message(
            "Use `/hxplayer <name> [region]`\n\n**Example:** `/hxplayer faker kr`",
            ephemeral=True
        )
...
class AccountSubMenu(ui.View):
    ...
    async def daily_claim(self, interaction: discord.Interaction, button: ui.Button):
        await interaction.response.send_message(
            "Use `/hxdaily` to claim your 100 tokens!",
            ephemeral=True
        )
    ...
    async def view_stats(self, interaction: discord.Interaction, button: ui.Button):
        await interaction.response.send_message(
            "Use `/hxstats` to view your betting statistics",
            ephemeral=True
        )
...
class InfoSubMenu(ui.View):
    ...
    async def help_button(self, interaction: discord.Interaction, button: ui.Button):
        await interaction.response.send_message(
            "Use `/hxhelp` for full documentation",
            ephemeral=True
        )
